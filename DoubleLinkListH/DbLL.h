...
public:
	...
	void LocateElem(const ElemType& e);	// 元素定位,访问度递减排列 
};
...
template <class ElemType>
void DblLinkList<ElemType>::Show() const
{
	DblNode<ElemType>* p;
...
template <class ElemType>
void DblLinkList<ElemType>::LocateElem(const ElemType& e)
{
	DblNode<ElemType>* p = head->next;
	...
	for(q = p->prior; q != head && q->freq < p->freq; q = p->prior)
	{
		p->prior = q->prior;
		q->next = p->next;
		p->next->prior = q;
