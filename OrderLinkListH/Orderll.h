...
public:
	...
	void Inverse();						//逆置
	void LocateElem(int i) const;	         // 求地址
	void CountE(const ElemType& e)const;	//统计e的个数	

	void InsertElemE(const ElemType& e);	//插入元素e
	void DeleteElemE(const ElemType& e);			//删除元素e
};
...
template <class ElemType>
void LinkList<ElemType>::LocateElem(int i) const
// 第i个节点的地址
{
	cout << "第" << i << "个结点的地址为 ";
...
template <class ElemType>
void LinkList<ElemType>::CountE(const ElemType& e)const
{
	Node<ElemType>* p = head->next;
	int count = 0;
	while (p != NULL)
	...
	cout << "链表中" << e << "的个数为：" << count << endl;
}

template <class ElemType>
void LinkList<ElemType>::InsertElemE(const ElemType& e)
{
	Node <ElemType>* p =head,*q;
	while (p->next != NULL)
	{
...
template <class ElemType>
void LinkList<ElemType>::DeleteElemE(const ElemType& e)
{
	Node <ElemType>* p = head, * q;
	while (p->next != NULL)
	{
